"""
Energy landscape (single-cell performance) for two archetypes, beta = 1.

Renders the single-cell fitness field
    F(x) = prod_j p(||x - a_j||) ,   p(d) = exp(-beta * d^gamma)
over 2D gene-expression space, side by side for gamma = 0.5 (sub-exponential,
cusped) and gamma = 1.5 (smoother, rounder). Uses core.performance directly.

Run from the repo root:
    python "paper figures/energy landscape/energy_landscape_figure.py"
"""
...
OUT_PATH = os.path.join(HERE, "energy_landscape_gamma_0p5_1p5.png")


def performance_field(gamma):
...
def main():
    fields = [performance_field(gamma) for gamma in GAMMAS]
    ...
    cf = None
    for ax, gamma, (gx, gy, F) in zip(axes, GAMMAS, fields):
        ...
        ax.set_title(rf"$\gamma = {gamma:g}$", fontsize=15)
    ...
    fig.suptitle(rf"Energy landscape (single-cell performance), $k = 2$, $\beta = {BETA:g}$",
                 fontsize=16)
